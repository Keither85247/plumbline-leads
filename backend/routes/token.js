...
// Requires an active session — identity is `user_<userId>` so each user
// registers as a unique Twilio Client and receives only their own inbound calls.
router.get('/', requireAuth, (req, res) => {
  ...
  try {
    const identity = `user_${req.userId}`;
    ...
    console.log(`[Token] Voice token issued for identity: ${identity}`);
